...
static jbyteArray android_os_Parcel_marshall(JNIEnv* env, jclass clazz, jlong nativePtr)
{
    ...
    if (ret != NULL)
    {
        ...
        if (array != NULL)
        {
            LOGI("chengang, android_os_Parcel_marshall");
            LOGI("chengang, android_os_Parcel_marshall parcel->dataSize(): %d", parcel->dataSize());
            //android::CallStack cs("***********memcpy, chengang***********")
